// Bead 2.5 — Suggestion persistence service
import { Database } from "bun:sqlite";
import { randomUUID } from "crypto";
import type { Suggestion, SuggestionRequest, SuggestionStatus } from "./suggestion-types";
...
export class SuggestionService {
  ...
  transition(
    ...
    const now = new Date().toISOString();
    const decisionStatuses: SuggestionStatus[] = [
      "accepted",
      "rejected",
      "edited_applied",
    ];
    ...
    this.db
      .prepare(
        `UPDATE suggestions
         SET status = ?, edited_text = ?, updated_at = ?, decided_at = COALESCE(?, decided_at)
         WHERE id = ?`
      )
      .run(status, editedText ?? null, now, decidedAt, id);
