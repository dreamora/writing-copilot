...
3. discoverable logged-in `codex` CLI → `CodexSuggestionProvider` (via `codex exec`, no API key required)
4. API-key auth when Codex is unavailable → `OpenAiSuggestionProvider`
