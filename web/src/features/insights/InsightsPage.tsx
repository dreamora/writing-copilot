// Bead 4.1 + 4.2 — Insight cockpit shell with data loading
...
export default function InsightsPage() {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      const [acc, tim, hot] = await Promise.all([
  ...
  const hasData = acceptance.length > 0 || time.length > 0 || hotspots.length > 0;
  ...
  return (
    <div style={{ maxWidth: "960px", margin: "0 auto", padding: "24px", fontFamily: "sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "#6b7280", margin: 0 }}>
            Writing session analytics and AI effectiveness signals
          </p>
        </div>
        {hasData && (
          <ExportSummaryButton
      ...
      {!loading && !error && hasLoaded && !hasData && (
        ...
        >
          ...
          <div style={{ fontSize: "12px", marginTop: "4px" }}>
            Start writing with the editor, then refresh.
          </div>
        </div>
      )}
      ...
      {hasLoaded && hasData && (
        <>
          {/* Helped/slowed banner — full width */}
